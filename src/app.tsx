...
const MemberList: FC<{ members: {state_key: string}[] }> = ({members}) => {
    if (members.length === 0) {
...
const MembersExplorer: FC<{ identity: Identity, roomId: string }> = ({identity, roomId}) => {
    const [busy, setBusy] = useState(false);
    const [members, setMembers] = useState<null | z.infer<typeof zMembers>>(null);
    ...
    return <>
        ...
        {groups && <>
            <details open>
                <summary><h3>Joined ({groups.get('join')?.length ?? 0})</h3></summary>
                <MemberList members={groups.get('join') ?? []} />
            </details>
            <details open>
                <summary><h3>Invited ({groups.get('invite')?.length ?? 0})</h3></summary>
                <MemberList members={groups.get('invite') ?? []} />
            </details>
            <details>
                <summary><h3>Knocking ({groups.get('knock')?.length ?? 0})</h3></summary>
                <MemberList members={groups.get('knock') ?? []} />
            </details>
            <details>
                <summary><h3>Left ({groups.get('leave')?.length ?? 0})</h3></summary>
                <MemberList members={groups.get('leave') ?? []} />
            </details>
            <details>
                <summary><h3>Banned ({groups.get('ban')?.length ?? 0})</h3></summary>
...
export const MainRouter: FC<{identity: Identity, roomId: string, subpage: string}> = ({ identity, roomId, subpage }) => {
    if (roomId.startsWith('!')) {
        if (subpage === 'yaml') {
            ...
            return <AdvancedRoomUpgradePage
                identity={identity}
                roomId={roomId}
            />;
        } else if (subpage === '') {
            ...
            return <>
                <AppHeader
                    backUrl={`#/${encodeURIComponent(identity.name)}/${encodeURIComponent(roomId)}`}
                >Unknown subpage</AppHeader>
                <main>
                    <p>404: Unknown subpage</p>
                </main>
            </>;
        }
